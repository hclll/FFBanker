...
from banking import cluster_and_merge_flip_flops, create_pin_mapping, resolve_overlaps, banking_each_clock_net, find_closest_row, calculate_placement, find_adjacent_empty_site
...
def debanking_some(die, cell_lib, netlist, decreased_slack,parsed_data): # decreased_slack is a dictionary
        ...
        if inst.cell_type in cell_lib.flip_flops:        
            ...
            if ff.bits > 1 and in_dict: 
                ...
                for bit in range(ff.bits):
                    ...
                    new_inst = Instance(new_name, single_bit_ff_type, inst.x, inst.y) # Make y the same so that it can trigger overlap that can be fixed.
                    ...
                    debanked_instances[new_name] = new_inst;
    ...
    # Place debanked instances into legal bottom-left coords if not already in legal location.
    placement_rows = parsed_data.placement_rows
    #print(placement_rows);

    #for inst in debanked_instances:
        #at_legal_y = False;
        #for row in placement_rows:
            #if inst.y == row[1]:
                #at_legal_y = True
        #if not at_legal_y:
            #closest_row = find_closest_row(inst.y,placement_rows);
